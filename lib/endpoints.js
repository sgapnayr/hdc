...
export const createEmployee = async (firstName, lastName, email, phone, role, employeeId) => {
  ...
  const result = await API.graphql({
    query: gql`
      mutation MyMutation($firstName: String!, $lastName: String!, $email: String!, $role: String!, $employeeId: ID!) {
        createEmployee(employee: { email: $email, firstName: $firstName, lastName: $lastName, phone: "123", role: PROVIDER, employeeId: $employeeId }) {
          firstName
          lastName
          email
          role
