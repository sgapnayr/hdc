...
export const getAllTasksByAssignee = async (assigneeId, limit = null, nextToken = null) => {
  const result = await API.graphql({
    query: gql`
      query GetTasksByAssignee($assigneeId: String!, $limit: Int, $nextToken: String) {
        getTasksByAssignee(assigneeId: $assigneeId, limit: $limit, nextToken: $nextToken) {
          tasks {
            type
            taskId
            updatedAt
    ...
    variables: {
      ...
      limit: limit, // Passing the limit to the GraphQL query
      nextToken: nextToken, // Passing the nextToken to the GraphQL query
    },
    authMode: 'AMAZON_COGNITO_USER_POOLS',
  })

  // The result now also contains a nextToken that you can use for fetching the next page
  return {
    tasks: result.data.getTasksByAssignee.tasks,
    nextToken: result.data.getTasksByAssignee.nextToken,
  }
}
...
export const getNotes = async (patientId, nextToken = null, limit = 10) => {
  // Assuming the default limit is 10, you can adjust this as needed
  const result = await API.graphql({
    query: gql`
      query MyQuery($patientId: String!, $nextToken: String, $limit: Int) {
        getNotesByPatient(patientId: $patientId, nextToken: $nextToken, limit: $limit) {
          nextToken
          notes {
            createdAt
            creatorId
    ...
    variables: {
      patientId,
      nextToken,
      limit, // Passing the limit to the GraphQL query
    },
    authMode: 'AMAZON_COGNITO_USER_POOLS',
  })
