...
export const createEmployee = async (firstName, lastName, email, phone, role, employeeId) => {
  ...
  const result = await API.graphql({
    query: gql`
      mutation MyMutation($firstName: String!, $lastName: String!, $email: String!, $phone: String!, $role: [Role!], $employeeId: ID!) {
        createEmployee(employee: { email: $email, firstName: $firstName, lastName: $lastName, phone: $phone, role: 0, employeeId: $employeeId }) {
          firstName
          lastName
          email
          role
